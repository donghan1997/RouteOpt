...
namespace RouteOpt::Branching::BBT {
    ...
    void BBTController<Node, BrCType, Hasher>::solve(Node *root_node,
        ...
        while (!tree.empty()) {
            ...
            SAFE_SOLVER(node->refSolver().getRhs(3*dim-1+1, 4, rhs_m_n.data()))
            std::cout << "solver node's m bounds: [" << rhs_m_n[0] << ", " << rhs_m_n[1] << "]" << std::endl;
            std::cout << "solver node's n bounds: [" << rhs_m_n[2] << ", " << rhs_m_n[3] << "]" << std::endl;
            ...
            std::cout << "current node's last brc = " << edge.first << ", " << edge.second
                      << ", dir = " << dir << ", tree_level = " << tree_level << std::endl;
            ...
            // std::cout << "Node value after pricing: " << valueExtractor(node) << std::endl;
            ...
            if (!branch_once) {
                ...
                if (!isTerminate(node)) {
                    ...
                    if (tree_level != 0) {
                        std::cout << "Updating r_star via LP testing." << std::endl;
                    ...
                    // std::cout << " BKFControllers size = " << bkf_controllers.size() << std::endl;
                    for (int i = 0; i < bst_ks.size(); ++i) {
                        // std::cout << "BKFController " << i << " is processing." << std::endl;
                        // Set the BKF controller state based on the current enumeration state.
                        // if (enuStateExtractor(node))
                        //     // std::cout << "BKFController " << i << " is in enumeration state." << std::endl;
                        // else
                        //     std::cout << "BKFController " << i << " is not in enumeration state." << std::endl;
                    ...
                    //     std::cout << "Using self-defined candidate selection function." << std::endl;
                    //     brc = getBestCandidateBySelfDefined(node, branching_history, branching_data_shared,
                    //                                         branching_tester);
                    // } else {
                    // std::cout << "Using default candidate selection function." << std::endl;
                    brc = branching_tester.getBestCandidate(node, branching_history, branching_data_shared,
                                                                getBranchingCandidates(node));
                    // }
                    // print the selected branching candidate.
                    std::cout << "Selected branching candidate: " << brc.first << ", " << brc.second << std::endl;
                    if (!brc.first && !brc.second) {
                        std::cout << "No candidate to be branched, Node is terminated, updating BKF shared data and deleting the node." << std::endl;
                        exit(0);
                        if (!bkf_controllers.empty()) {
                        ...
                        for (auto &child: children) {
                            ...
                            // std::cout << "Add the child node to the tree for further exploration." << std::endl;
                            addNode(child);

                            std::vector<double> rhs_m_n(4);
                            SAFE_SOLVER(child->refSolver().getRhs(3*dim-1+1, 4, rhs_m_n.data()))
                            std::cout << "child index = " << idxExtractor(child) << std::endl;
                            std::cout << "child node's m bounds: [" << rhs_m_n[0] << ", " << rhs_m_n[1] << "]" << std::endl;
                            std::cout << "child node's n bounds: [" << rhs_m_n[2] << ", " << rhs_m_n[3] << "]" << std::endl;
                    ...
                    // branch_once = true; // Set the flag to true after branching once.
                } else {
                    // If the node is terminated, update BKF shared data and delete the node.
                    std::cout << "Node is terminated, updating BKF shared data and deleting the node." << std::endl;
                    if (!bkf_controllers.empty()) {
                        bkf_data_shared.updateF(ub_ref.get() - valueExtractor(node));
                    }
                    delete node;
